...
if (-not $SkipFunctionalTest) {
    ...
    if ($functionalReport -and $functionalReport.available -and -not $functionalReport.correctness_ok) {
        throw 'Numerical functional validation failed. This runtime must not be treated as safe/validated for workloads.'
    }
} else {
    Write-Host ''
...
$functionalPass = [bool]($functionalReport -and $functionalReport.available -and $functionalReport.correctness_ok)

Write-Host ''
if ($functionalPass) {
    Write-Host 'READY - runtime smoke + numerical functional checks passed'
} elseif ($referenceGpu) {
