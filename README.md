...
On the current `gfx1200` ZLUDA v7 + TheRock test stack, a generated proxy preserved **940/940 exports** with **75 AMD-native hipSOLVER overrides**. `torch.linalg.solve` passed CPU-reference checks for FP32, FP64, complex64 and complex128. The Cholesky probe also passed `torch.linalg.cholesky`, `torch.cholesky_solve` and `torch.cholesky_inverse` for those four dtypes in both single-matrix and batched tensor cases. QR coverage passes `torch.linalg.qr`, `torch.geqrf`, `torch.orgqr`/`ungqr`, and `torch.ormqr`/`unmqr` for the same four dtypes, including batched matrices. The ZLUDA BLAS patch additionally routes classic and strided-batched FP64/complex GEMM to rocBLAS and bridges cuBLAS batched GEQRF to hipSOLVER generic-X; the tested QR matrix covers default and reused non-default CUDA streams. The original cuSOLVER DLL was restored with the exact same SHA-256 after validation. This is an incremental compatibility layer, not a claim that every cuSOLVER routine is AMD-native yet.
...
The experimental v7/TheRock path also has a same-GPU native baseline. `scripts/benchmark-gemm.ps1` compares FP32 SGEMM through direct HIP/rocBLAS with the same operation through PyTorch CUDA → ZLUDA → the AMD BLAS path. Because Windows HIP event timing was independently observed returning invalid negative intervals, the benchmark's authoritative metric is synchronized monotonic wall-clock time; HIP/CUDA event timings are retained only as diagnostics. Execution order alternates direct-first and ZLUDA-first, and the runner reports the median of paired deltas. On the final clean-patch four-pair validation run, paired median overhead was **+3.07% at 1024²**, **+16.94% at 2048²**, and **+3.74% at 4096²**. All three sizes remained inside the repository's 20% regression budget; paired throughput ratios were **97.0%**, **85.5%**, and **96.4%** respectively. The 2048² result showed the widest run-to-run variation in this sample, so the JSON retains every pair rather than presenting a single timing as universal. Individual samples still vary with clocks/thermals, so the JSON retains every pair. This is a same-GPU AMD-native baseline, not a comparison with an NVIDIA GPU.

A real VelocityRL `512 agents × rollout 16` three-update smoke on the final clean-patch runtime recorded **512 SPS** for the non-representative cold first update, then **72,135 SPS** and **70,822 SPS** for the warmed updates, giving **71,478.5 SPS median steady-state**. The cold figure varies sharply with first-use compilation/cache state, so the report stores it separately instead of treating it as a throughput benchmark.
