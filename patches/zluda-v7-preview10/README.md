...
- **Separate experimental cuDNN convolution bridge**: the repository also contains a source-built top-level `cudnn64_8.dll` proxy that preserves the user's original export surface and routes a narrow legacy cuDNN v8 2D forward + backward-data + backward-filter subset to MIOpen. On the RX 9060 XT / gfx1200 reference system with TheRock 7.14.0 and PyTorch 2.3.0+cu118, the versioned matrix passes basic/padding/stride/dilation/grouped cases in FP32 and FP16 for forward, input gradients and weight gradients against CPU references. It remains separate from the ZLUDA source patch and stable installer; see `docs/CUDNN_BRIDGE.md`.
